...
| Act 4 / generazione | Citazione troncata + qualifica fonti | Il modello citava ID abbreviati (es. `[comitato_palermo_v2]` invece di `[nota_comitato_palermo_v2]`) e le fonti non neutre (propaganda, obsolete, contested...) rischiavano di essere presentate come fatti neutri | L'istruzione di prompt "copia l'ID esattamente" da sola non bastava — il modello tendeva comunque ad abbreviare ID verbosi in generazione | Aggiunta rete di sicurezza deterministica `fix_citation_ids()`: dopo la generazione, ogni `[id]` nel testo che non corrisponde a un document_id reale viene confrontato per suffisso con i document_id dei contexts e corretto in automatico; inoltre il campo `reliability` del manifest (propaganda/obsolete/corrective/harmful_if_unqualified/contested/allusive/incomplete) è stato collegato ai metadati di ingest e al prompt di grounding, che ora qualifica esplicitamente le fonti non neutre | Risolto e verificato su 4 domande di test mirate (version-awareness, propaganda, harmful_if_unqualified, contested): citazioni sempre esatte per intero, qualificazione delle fonti presente in tutte le risposte (con un margine di imprecisione su quale fonte specifica riceve quale giudizio, osservato su un test) |
...
-  **Version awareness / propaganda**: ✅ Fatto — il prompt usa il campo `reliability` del manifest per qualificare esplicitamente le fonti non neutre, verificato su 4 test mirati Act 4 (vedi tabella fix sopra). Precisione dell'attribuzione (quale fonte specifica riceve quale giudizio) da tenere d'occhio, non perfetta al 100%.
- **Retrieval diversity per documento**: con Act 4 il corpus è cresciuto e in un test il documento più pertinente è arrivato solo al rank 5/5, con 2 slot su 5 finiti su chunk diversi ma sovrapposti dello stesso documento — nessun impatto negativo osservato finora, ma da monitorare se capita anche sulle domande reali di Round 2 (eventualmente alzare `k_retrieve` o aggiungere un limite "max N chunk per documento" nel dedup).